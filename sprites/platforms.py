...
class Platform(pygame.sprite.Sprite):
  ...
  def update(self): 
    ...
    if self.player.rect.top < HEIGHT/2 and not self.is_scrolling_down and not self.player.is_falling and not self.is_scrolling_up:
...
class Monster(Platform):
  def __init__(self, x, y, player, platform):
    Platform.__init__(self, x, y, player)
  ...
  def monster_update(self):
    ...
    self.stop_monster_music()
    self.kill_monster()
    self.kill_player()
    self.rect.centerx = self.platform.rect.centerx
  
  def start_monster_music(self):
    if not self.is_music_playing and (self.player.rect.top - self.rect.bottom) <= 1.25*HEIGHT:
      monster_sound.play(100)
      self.is_music_playing = True
  
  def kill_monster(self):
    if len(pygame.sprite.spritecollide(self, self.player.bullets, False)) != 0:
      monster_sound.stop()
      hit_sound.play()
      self.kill()
  
  def stop_monster_music(self):
    if self.player.is_loose or self.rect.top >= HEIGHT:
      monster_sound.stop()
      self.is_music_playing = False
  
  def kill_player(self):
    if len(pygame.sprite.spritecollide(self.player, [self], False, pygame.sprite.collide_circle_ratio(0.7))) != 0 and not self.player.is_loose and not self.player.is_falling:
...
class Platforms(pygame.sprite.Sprite):
  ...
  def generate_platforms(self):
    ...
    for i in range (1, 500):
      ...
      if i % monster_frequency == 0:
        monster = Monster(current_x + platform_size[0]/2, y, self.player, platform)
